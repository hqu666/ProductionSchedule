    ...
    class DBViewModel : INotifyPropertyChanged {
     ...
     //   static string ApplicationName = Constant.ApplicationName;           //"Google Sheets API .NET Quickstart";
       ...
       // static string SpreadSheetId = "(操作対象となるシートのID)";
        ...
        /// <summary>
        /// Drag移動を追跡し、通過するアイテムとの位置関係をグローバル変数に保持させる
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void DragOverTree(object sender, DragEventArgs e , Grid HierarchyGrid) {
            string TAG = "DragOverTree";
            string dbMsg = "";
            try {
                //// 背景色やセパレータを元に戻します
                            ...
                            if (targetElementInfo == sourceItem) {
                                ...
                                if (targetElementInfo == null) {
                                    ...
                                    if (targetElementInfo.ContainsParent(sourceItem)) {
                                        ...
                                        dbMsg += ",drop先[" + targetElementInfo.id + "]" + targetElementInfo.name;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        /// <param name="HierarchyGrid"></param>
        public void DropInTree(object sender, DragEventArgs e) {
            string TAG = "DropInTree";
            string dbMsg = "";
            try {
